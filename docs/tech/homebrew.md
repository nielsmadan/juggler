...
The checksum is computed from the final notarized DMG. The release workflow needs its existing `HOMEBREW_TAP_TOKEN` with write access to `homebrew-tap`. Follow the repository's `just tag-release-patch` workflow after committing changes to publish a new app and cask together; editing the template alone does not update installed users.
