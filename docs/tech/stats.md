...
Implementation of the busy-time tracking subsystem: per-session accrual, per-day persistence, and the chart/corner-tab layout. For the user-facing product view (what the chart looks like and the settings that control it), see [docs/features/main-window.md](../features/main-window.md).
