...
class HomebrewReleaseTests(unittest.TestCase):
    def setUp(self):
        ...
        self.helper = self.app / "Contents/MacOS/hooklinesinker"
    ...
    def test_rejects_a_missing_or_nonexecutable_embedded_hooklinesinker(self):
        self.helper.chmod(0o644)
        with self.assertRaisesRegex(ValueError, "Missing executable embedded hooklinesinker"):
            renderer.render_cask("1.7.3", self.app, self.dmg)
        self.helper.unlink()
        with self.assertRaisesRegex(ValueError, "Missing executable embedded hooklinesinker"):
            renderer.render_cask("1.7.3", self.app, self.dmg)
